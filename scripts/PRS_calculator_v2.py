...
def analyse_anc(merged_phase_copyprobs_temp, anc, chrom, pval, LD_value, phenotype_file):
    ...
    for i in list_of_SNPs:
        ...
        if minor_allele == alt_allele:
            ...
            if phase_sum_dict[i] > (len(haps) / 2):
                alt_mapping[i] = 0
            else:
                #  major is alt
                alt_mapping[i] = 1
            ...
            if phase_sum_dict[i] > (len(haps) / 2):
                ...
                alt_mapping[i] = 0
    output = pd.DataFrame(index=['alt', 'ref'], columns=list_of_SNPs)
    for i in list_of_SNPs:
        ...
        output.at['alt', i] = alt_sum
        output.at['ref', i] = ref_sum
    output = output.T
...
for file in phenotypes:
    ...
    for p in [0.01, 0.05, 0.1]:
        ...
        for LD in ["05.merged.filtered.prune.in", "5.merged.filtered.prune.in", "9.merged.filtered.prune.in"]:
            ...
            phase_temp['haps'] = haps
            anc_copyprobs_temp1 = anc_copyprobs[list_of_SNPs]
            anc_copyprobs_temp1 = anc_copyprobs_temp1.reset_index().rename(columns={'0': 'ID'})
            anc_copyprobs_temp1['haps'] = haps
            merged_phase_copyprobs = pd.merge(phase_temp, anc_copyprobs_temp1, on=['ID', 'haps'])
