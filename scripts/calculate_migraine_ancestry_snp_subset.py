...
mapping = pd.read_csv("/willerslev/ukbiobank/migraine_analysis/rsID-chr_mapping", header=None, sep="\t", dtype=str)
rsID_list = mapping[mapping[2] == str(args.chr)][0].tolist()
rsID_files = list()
for rsID in rsID_list:
    rsID_files.append("/willerslev/ukbiobank/migraine_analysis/output_files/" + str(rsID) + ".0.haps")
...
for anc in ["CHG", "EHG", "Farmer", "African", "EastAsian", "WHG", "Yamnaya"]:
    ...
    for rsID in rsID_list:
        if not os.path.exists("/willerslev/ukbiobank/migraine_analysis/output_files/" + rsID + ".0.haps"):
            continue
        print("Processing " + str(rsID))
        position_mapping = pd.read_csv("/willerslev/ukbiobank/migraine_analysis/rsID-position_mapping", sep="\t", header=None)
