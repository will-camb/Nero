...
for i in range(1,23):
  ...
  dummy.to_csv("new." + str(i) + ".master_all_copyprobsperlocus.txt", header=False, index=False)
...
chunklengths.to_csv("new.ordered_all_pop_ids_mapped.allchr.chunklengths.out", sep=" ", header=False, index=False)
