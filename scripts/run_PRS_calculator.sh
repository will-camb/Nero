...
for chr in $chrlist; do
  for anc in $ancestries; do
    echo "python3 PRS_calculator.py -copyprobs_file $copyprobs_directory/temp.$anc.$chr.master_all_copyprobsperlocus.txt.gz -phasefile $phasefile_directory/transformed.$chr.merged.phase.gz -idfile $idfile -phenotypes $phenotype_file -pruned_dir $pruned_dir -chr $chr -anc $anc" >> PRS_calculator_commands
  done
done

echo "Now running commands in PRS_calculator_commands in parallel"
cat PRS_calculator_commands | parallel  -j 60
