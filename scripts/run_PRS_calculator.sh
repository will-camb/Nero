...
echo "*** Extracting results files from Dropbox ***"
...
##                              header=None,
##                              index_col=0)[2].dropna().apply(lambda x: pd.Series(list(x))).apply(pd.to_numeric)
#  copyprobsFarmer = pd.read_csv("temp.FarmerAnatolian."+str(i)+".master_all_copyprobsperlocus.txt.gz",
#                              header=None,
#                              index_col=0)[2].dropna().apply(lambda x: pd.Series(list(x))).apply(pd.to_numeric) + \
#                              pd.read_csv("temp.FarmerEarly."+str(i)+".master_all_copyprobsperlocus.txt.gz",
#                              header=None,
#                              index_col=0)[2].dropna().apply(lambda x: pd.Series(list(x))).apply(pd.to_numeric) + \
#                              pd.read_csv("temp.FarmerLate."+str(i)+".master_all_copyprobsperlocus.txt.gz",
#                              header=None,
#                              index_col=0)[2].dropna().apply(lambda x: pd.Series(list(x))).apply(pd.to_numeric) + \
#                              pd.read_csv("temp.FarmerMiddle."+str(i)+".master_all_copyprobsperlocus.txt.gz",
#                              header=None,
#                              index_col=0)[2].dropna().apply(lambda x: pd.Series(list(x))).apply(pd.to_numeric)
...
#NEW_EXT=${MY_FILE/bgz/gz}
NEW_EXT=$line
for p_val in {0.01,0.05,0.1}; do
for pruned_in in {05.merged.filtered.prune.in,5.merged.filtered.prune.in,9.merged.filtered.prune.in}; do
echo "python3 PRS_calculator.py -copyprobs_directory $copyprobs_directory -phasefile_directory $phasefile_directory -idfile $idfile -file_name $NEW_EXT -pruned_dir $pruned_dir -pruned_in $pruned_in -p_value $p_val" >> PRS_calculator_commands
done
done
done < "$phenotype_file"
